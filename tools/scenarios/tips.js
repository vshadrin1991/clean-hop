...
({ tvs, byKind: t.offByKind, tipAfterTVs, tipEmpty,
   pass: tvs >= 2 &&
         tipAfterTVs === 'Никто не смотрит? Выключи телевизор совсем — пульт оставляет его дремать.' &&
         ['Открываешь холодильник — закрывай дверцу побыстрее, пусть холод не убегает!',
          'Горячая вода — это тоже энергия. Закрывай кран, пока чистишь зубы.',
          'Приборы «спят», но всё равно едят энергию. Выключай их кнопкой.']
           .includes(tipEmpty) })
